...
export const getCategoryById = async (req: Request, res: Response) => {
  const [rows] = await db.query('SELECT * FROM categorias WHERE id = ?', [req.params.id]) as [any[], any];
  res.json(rows[0]);
};
...
export const updateCategory = async (req: Request, res: Response) => {
  const { name }: Category = req.body;
  await db.query('UPDATE categorias SET name = ? WHERE id = ?', [name, req.params.id]);
  res.sendStatus(204);
};
